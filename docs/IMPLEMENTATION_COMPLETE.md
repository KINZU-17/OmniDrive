...
## 🏢 Multi-Dealership Support (May 2026 Update)

Additional features for multi-dealership marketplace:

### 5. ✅ Multi-Dealership Architecture
**Requirement:** Support multiple independent dealerships on one platform

**Implementation:**
- Created `DEALERSHIPS.md` with complete architecture documentation
- Each dealership has its own admin who manages ONLY their inventory
- Super Admin controls the entire platform
- Clients can browse vehicles from ALL dealerships
- Vehicles tagged with `dealership_id` for proper scoping

### User Roles in Multi-Dealership Context:
| Role | Scope | Access |
|------|-------|--------|
| **Super Admin** | Platform-wide | All dealerships, global stats |
| **Dealership Admin** | Single dealership | Only their vehicles, staff, customers |
| **Dealership Staff** | Single dealership | Limited to assigned dealership |
| **Technical Liaison** | Cross-dealership | Can work with multiple dealerships |
| **Client** | All dealerships | Browse all, contact specific dealerships |
...
ALTER TABLE users ADD COLUMN dealership_id INTEGER;
ALTER TABLE vehicles ADD COLUMN dealership_id INTEGER NOT NULL;
```
...
GET  /api/dealerships/:slug/vehicles     # Get dealership vehicles
POST /api/vehicles                       # Create (scoped to user's dealership)
PUT  /api/vehicles/:id                   # Update (if owned by user's dealership)
```

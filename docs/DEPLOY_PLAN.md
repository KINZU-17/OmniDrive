...
**Single Service Architecture**: Deploy `server.js` from project root as combined backend+frontend
...
| File | Purpose |
|------|---------|
| `scripts/DEPLOY_ALL.sh` | Full automated deployment script |
...
Run `./scripts/DEPLOY_ALL.sh` or follow manual steps above.
